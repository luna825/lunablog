...
class APITestCase(unittest.TestCase):
	...
	def test_token_auth(self):
		...
		response = self.client.get(url_for('api.get_posts'),
			headers = self.get_api_headers('jhon111@example.com',password='cat'))
		self.assertTrue(response.status_code==401)
		response = self.client.get(url_for('api.get_posts'),
			headers = self.get_api_headers('bad_token',''))
		self.assertTrue(response.status_code==401)

		response = self.client.get(url_for('api.get_token'),
			headers = self.get_api_headers('jhon@example.com',password='cat'))
	...
	def test_unconfirmed_account(self):
		r = Role.query.filter_by(name='User').first()
		self.assertIsNotNone(r)
		u = User(email='jhon@example.com',password='cat',confirmed= False,role=r)
		db.session.add(u)
		db.session.commit()
	...
	def test_posts(self):
		...
		response = self.client.post(
		    url_for('api.new_post'),
		    headers=self.get_api_headers('john@example.com', 'cat'),
		    data=json.dumps({'body': 'api body of the *blog* post.'}))
		...
		self.assertTrue(json_response['body'] == 'api body of the *blog* post.')
		self.assertTrue(json_response['body_html'] ==
		                '<p>api body of the <em>blog</em> post.</p>')
		...
		#get posts form user as a flower
		response = self.client.get(url_for('api.get_user_followed_posts',id=u.id),
			headers=self.get_api_headers(u.email,'cat'))
		self.assertTrue(response.status_code==200)
		json_response = json.loads(response.data.decode('utf-8'))
		self.assertIsNotNone(json_response.get('posts'))
		self.assertTrue(json_response.get('count',0)==1)
		...
		json_response = json.loads(response.data.decode('utf-8'))
		self.assertTrue(json_response['url'] == url)
		self.assertTrue(json_response['body'] == 'update body')
		self.assertTrue(json_response['body_html'] == '<p>update body</p>')

	def test_users(self):
		# add two users
		u1 =User(username= 'john',password='cat')
		u2 = User(username='suan',password='dog')
		db.session.add(u1)
		db.session.add(u2)
		db.session.commit()
		...
		self.assertTrue(response.status_code==200)
		json_response = json.loads(response.data.decode('utf-8'))
		self.assertTrue(json_response['username']=='suan')
